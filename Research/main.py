...
env_height = 40
env_width = 40
board = Board(env_width, env_height)
...
def random_destination(position):
    if position[0] < env_width/4:
        destination_x = random.randint(0, math.floor(position[0] + env_width/4))
    elif position[0] + env_width/4 > env_width:
        destination_x = random.randint(math.floor(position[0] - env_width/4), env_width-1)
    else:
        destination_x = random.randint(math.floor(position[0] - env_width / 4), math.floor(position[0] + env_width/4))

    if position[1] < env_height/4:
        destination_y = random.randint(0, math.floor(position[1] + env_height/4))
    elif position[1] + env_height/4 > env_height:
        destination_y = random.randint(math.floor(position[1] - env_height/4), env_height-1)
    else:
        destination_y = random.randint(math.floor(position[1] - env_height / 4), math.floor(position[1] + env_height/4))
...
#     observer.append(Observer(i, (0, 0)))
observer.append(Observer(0, (30, 20)))
observer.append(Observer(1, (20, 30)))
# observer.append(Observer(1, (39, 39)))

target = []
# for i in range(0, no_of_targets):
#     target.append(Target(i, (20, 20)))
target.append(Target(0, (20, 20)))
# target.append(Target(1, (27, 27)))
# target.append(Target(2, (0, 1)))
# target.append(Target(3, (1, 1)))
...
for timestep in range(0, 1500):
    ...
    if timestep % 10 == 0:
        for obs in observer:
            ...
            print('Observer', obs.oid, 'target list', obs.target_list)
            ...
            print('Observers destination', obs.get_destination())
    ...
    for targ in target:
        if targ.get_position() == targ.get_destination():
            ...
            print('Target', targ.tid, 'observer list', targ.tid, targ.obs_list)
            ...
            # Mean position of observers
                ...
                if direction_vector == (0, 0):
                    direction_vector = (random.randint(-1, 1), random.randint(-1, 1))
            ...
            print('Targets destination', targ.get_destination())


    # Clear prev position from board
    for targ in target:
        board.render_empty(math.floor(targ.get_position()[0]), math.floor(targ.get_position()[1]))
    for obs in observer:
        board.render_empty(math.floor(obs.get_position()[0]), math.floor(obs.get_position()[1]))
    ...
    # Print new board
    for obs in observer:
        print('Observers position', obs.get_position())
    for targ in target:
        print('Targets position', targ.get_position())
    for targ in target:
        board.render_target(math.floor(targ.get_position()[0]), math.floor(targ.get_position()[1]))
    for obs in observer:
        board.render_obs(math.floor(obs.get_position()[0]), math.floor(obs.get_position()[1]))
    board.print()
    input("Press a key")
