...
for exp in range(0, no_of_expts):
    ...
    for timestep in range(0, 1500):
        ...
        if timestep % 10 == 0:
            for obs in observer:
                # Update target list
                closest_target_distance = 200
                obs.target_list = []
                for targ in target:
                    dist = distance(obs.get_position(), targ.get_position())
                    if dist <= observer_sensor:
                ...
                # LSTP
                elif obs.lstp is not None:
                    obs.set_destination(obs.lstp)
                # Random
                else:
    ...
    # print(avg_no_of_targets_observed_per_time_step)
