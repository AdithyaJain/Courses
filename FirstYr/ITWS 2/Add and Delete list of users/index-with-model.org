...
<div id="get-all-users-container" class="hidden">
  <ul id="users-list">
    <li>
      Name: Steve Bannon | Email: bannon@whitehose.gov
...
*** Wire all the events to the the listeners. 
...
function add_user_handler() {
  ...
  $('#add-user-container #add')[0].addEventListener("click", addUserToSystem);
...
*** Define how a user is added 
    This function of how a user
    is added is added as a
    listener on the =click=
    event for the =Add= button.

#+NAME: add_user_to_system
#+BEGIN_SRC js
var addUserToSystem = function() {
...
* Model
  The model contains a list of users and operations on the list and user.  Each
  user is uniquely identified by email.  The attributes of a user are =name=
  and =email=.
...
var systemMaker = function() {
  ...
  var deleteUser = function(email) {
    let newList = filterByEmailCompliment(email);
    userList = newList;
  };
  ...
  system.addUser = addUser;
  system.userExists = userExists;
  system.findUserByEmail = findUserByEmail;
  system.deleteUser = deleteUser;
  system.updateUser = updateUser;
  return system;
};

var system = systemMaker();
